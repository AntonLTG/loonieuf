<script lang="ts">
  ...
  export let picColor: ColorTypes;

  const marinblåImages = [
    {
      id: 0,
  ...
  $: getImages = () => {
    if (picColor == "Mörkgrå") {
      return darkgreyImages;
    }
    if (picColor == "Ljusgrå") {
      return ljusgråImages;
    }
    if (picColor == "Bebis rosa") {
      return bebisrosaImages;
    }
    if (picColor == "Marin blå") {
      return marinblåImages;
    }
    return [];
  };
</script>
